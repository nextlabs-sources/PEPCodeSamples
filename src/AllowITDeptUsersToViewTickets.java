...
public class AllowITDeptUsersToViewTickets {
	...
	public static void main(String[] args) throws Exception {
		...
		String propertiesFile = "../config/openaz-pep-on-prem.properties";
		
		if(args.length>0){
			
			if (args[0].equalsIgnoreCase("cloudaz")){
				propertiesFile = "../config/openaz-pep.properties";
				System.out.println("Using cloundAZ profile");
			}
			else if (args[0].equalsIgnoreCase("embedded")){
				propertiesFile = "../config/openaz-pep-embedded.properties";
				System.out.println("Using embeded profile");
			}
		}
		 ...
		 * with PDP
		 */
		final File f = new File(AllowITDeptUsersToViewTickets.class.getProtectionDomain().getCodeSource().getLocation().getPath() + propertiesFile);		
		PepAgentFactory pepAgentFactory = new StdPepAgentFactory(f.toString());
		...
		/*
		 * Build an Application object with applicaton-id or application-name
		 */
		Application app = Application.newInstance("SampleApp");
		 ...
		 */
		PepResponse pepResponse = pepAgent.decide(user, action, resource, environment,app);
